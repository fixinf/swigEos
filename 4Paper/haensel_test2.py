...
iENkv = interp1d(NKV, EKV, kind=kind)
...
erange = np.linspace(E_s, 0, 200, endpoint=0)
...
iPN = interp1d(nrange, Pnew)
print('here')
iEP = interp1d(Pnew, erange)
n_for_p = np.linspace(min(nrange), max(nrange), 100)
Pofn = iPN(n_for_p)
Eofn = iEP(Pofn)
# iENnew = interp1d(nrange, Eofn)
# print(iENnew(0.5*wr.n0) * wr.mpi4_2_mevfm3, iENkv(0.5*wr.n0) * wr.mpi4_2_mevfm3)
# exit()
ecrust = iENcrust(n_for_p)
# plt.plot(n_for_p, Eofn/n_for_p)
# plt.plot(n_for_p, ecrust/n_for_p)
# plt.plot(n_for_p, iENkv(n_for_p)/n_for_p)
# plt.show()

np.savetxt('Ediff.dat', np.array([n_for_p/wr.n0, (Eofn - ecrust)*wr.mpi4_2_mevfm3]).transpose(),
           fmt='%.8f')
exit()
