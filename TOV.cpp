...
int eq_volkov_i(double r, const double y[], double f[], void *params) {
	bool debug = false;
	double A = 1.4766;
	double B = 1.0;
	double C = 0.0004898007281478712;
	...
	double mpi4 = 5.7207e-5;
	f[3] = mpi4*(8 * M_PI/3) * pow(r,4) * (1 - 2 * f[3] / pow(r,3)) * (1 - f[3] / 2 / pow(r,3)) *
			(y[2] + y[0]) / (1 - 2 * 1.4766 * y[1] / r);
...
void star_crust_i(double rho_init, double* result, int dimResult, DriverBase* D, double nmin) {
	gsl_odeiv2_system sys = { eq_volkov_i, NULL, 4, NULL};
	double delta = 0.;
	gsl_odeiv2_driver * d = gsl_odeiv2_driver_alloc_y_new(&sys,
														  gsl_odeiv2_step_rkf45, 1e-3, 1e-3, 1e-3);
	int i;
	...
	double P_init = D->PofN(rho_init);
	double E_init = D->EofN(rho_init);
	double y[4] = { D->PofN(rho_init), 1.333 * M_PI * pow(r_init, 3.0) * 1.4766,
					E_init, 0.};
	double f[4];
	double bMass = 0.;
	double Dim = pow(135.,4);
