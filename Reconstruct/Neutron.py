...
nrange2 = np.linspace(1e-11, 4.5, 100)
...
def func_f_solve(f, n):
    ...
    res *= (iF0(n) - Co/mn**2 - Cr/(4*mn**2))
    return res


# def func_f_solve_NM(f, n):
#     f = float(f)
#     print f, n
#     pf = eos.p_f(n/2)
#     mn = C.M[0]
...
f= 0.
for n in nrange:
    f, =eos.f_eq(np.array([n, 0.]), np.array([f]), 1, C2)
    f2.append(f)
...
def _E(n, fp):
    ...
    res += Cr*n**2 / (8 * C.M[0]**2)
    int = quad(lambda z:func_integr(z, fp), 0.0, pf(n))[0]
#     print 'int = ', int
    res += int
    return res*135
