...
class EnthalpySolver:
    ...
    def __init__(self, E=None, P=None, nhpoints=500):
...
#         print 'P(h)=', self.PofH(h), 'E(h)=', self.EofH(h)
    ...
    def setEos(self, E, P):
        ...
        func_h = interpolate.interp1d(P, np.nan_to_num(1./(P + E)), kind='cubic')
        self.h = lambda z: integrate.quad(func_h, P[0], z)[0]
        self.HofP = np.array(list(map(self.h, P)))
...
#         exit()
        ...
        MR = integrate.odeint(self.equation, [r0, 4*pi*r0**3*self.E_const*self.E[0]/3.],
                      hrange, rtol=1e-11, hmax=1e-3, tcrit=np.array([0., 0.]), full_output=0)
        # print(MR)
        return MR[:, 1], MR[:, 0], self.PofH(hrange), self.EofH(hrange), hrange
...
#         hc = self.HofP[-1]        
        ...
        hrange = np.linspace(0., hc, self.nhpoints)
        r0 = 1e-10
