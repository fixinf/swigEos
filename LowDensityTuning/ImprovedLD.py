...
etar_old = list(map(C2.eta_r, frange))
...
S = np.array(list(map(C2.s, frange)))
...
plt.plot(frange, list(map(C1.eta_r, frange)))
...
order = 3
plt.plot(frange, [derivative(C2.eta_r, z, dx=1e-3, n=order, order = 5) for z in frange])
plt.plot(frange, [derivative(C2.eta_o, z, dx=1e-3, n=order, order=5) for z in frange])
plt.plot(frange, [derivative(C2.eta_s, z, dx=1e-3, n=order, order=5) for z in frange])
plt.show()
