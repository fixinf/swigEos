...
for p in params:
    ...
    #           for z in zlist]
    models = [Models2.Cubero_cut(.3, K0=p['K'], f0=p['f0'], J0=J0, suffix='K=%3.0ff=%1.2f'%(p['K'], p['f0']))]
    frange = np.linspace(0, 1, 100)
    lines = []
    legend = []
    for m in models:
        E, P, n = m.sym.EPN()
        E, f = m.sym.E(n, ret_f=1)
        plt.plot(n/m.n0, 1-f, label='orig')
        m.sym.C.a_sigma = 0.1
        E, f = m.sym.E(n, ret_f=1)
        plt.plot(n/m.n0, 1-f, label='new')
        plt.show()
        # m.dumpUofE()
        # m.dumpUofE_anti()
        # m.sym.dumpJ()
